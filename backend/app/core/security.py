        ...
        try:
            if self.config_file.exists():
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    config = json.load(f)
                    # Utiliser une variable d'environnement pour le mot de passe par défaut
                    default_password = os.getenv('ADMIN_PASSWORD', 'admin123')
                    self.admin_password = config.get('admin_password', self.hash_password(default_password))
