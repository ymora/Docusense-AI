...
def get_format_statistics() -> Dict[str, int]:
    ...
    return {
        ...
        'total_formats': len(AUDIO_EXTENSIONS) + len(VIDEO_EXTENSIONS) + len(IMAGE_EXTENSIONS),
        'audio_mime_types': len(AUDIO_MIME_TYPES),
        'video_mime_types': len(VIDEO_MIME_TYPES),
        'image_mime_types': len(IMAGE_MIME_TYPES),
        'total_mime_types': len(AUDIO_MIME_TYPES) + len(VIDEO_MIME_TYPES) + len(IMAGE_MIME_TYPES)
    }
