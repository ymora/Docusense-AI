    ...
    persist(
      (set, get) => ({
        ...
        loadAIProviders: async () => {
          ...
          if (isInitialized) {
            console.log('🔑 Configurations déjà chargées en mémoire, utilisation du cache');
            return;
          }
          ...
          try {
            console.log('🔑 Chargement initial des configurations depuis le backend...');
            ...
            console.log(`🔑 ${response.providers?.length || 0} providers IA chargés et mis en cache`);
          } catch (error) {
        ...
        refreshAIProviders: (() => {
          ...
          return callGuard(async () => {
            ...
            try {
              console.log('🔑 Actualisation des configurations depuis le backend...');
              const response = await ConfigService.getAIProviders();
              ...
              console.log(`🔑 ${response.providers?.length || 0} providers IA actualisés`);
            } catch (error) {
        ...
        saveAPIKey: (() => {
          ...
          return callGuard(async (provider: string, apiKey: string) => {
            try {
              console.log(`🔑 Sauvegarde de la clé API pour ${provider}...`);
              ...
              if (result.success) {
                ...
                console.log(`🔑 Clé API sauvegardée pour ${provider}`);
              }
        ...
        testProvider: (() => {
          ...
          return callGuard(async (provider: string) => {
            try {
              ...
              console.log(`🔑 Test ${result.success ? 'réussi' : 'échoué'} pour ${provider}`);
        ...
        setProviderPriority: async (provider: string, priority: number) => {
          try {
            console.log(`🔑 Définition de la priorité ${priority} pour ${provider}...`);
            ...
            if (result.success) {
              ...
              console.log(`🔑 Priorité mise à jour pour ${provider}`);
            }
        ...
        setStrategy: async (strategy: string) => {
          try {
            console.log(`🔑 Définition de la stratégie ${strategy}...`);
