      ...
      (set, get) => ({
        ...
        loadPrompts: (() => {
          ...
          return callGuard(async () => {
            ...
            if (isInitialized) {
              console.log('📋 Prompts déjà chargés en mémoire, utilisation du cache');
              return;
            }
            ...
            try {
              console.log('📋 Chargement initial des prompts depuis le backend...');
              ...
              console.log(`📋 ${prompts.length} prompts chargés et mis en cache`);
            } catch (error) {
        ...
        refreshPrompts: async () => {
          ...
          try {
            console.log('📋 Actualisation des prompts depuis le backend...');
            const prompts = await promptService.getPrompts();
            const categories = promptService.getPromptCategories();
            ...
            console.log(`📋 ${prompts.length} prompts actualisés`);
          } catch (error) {
