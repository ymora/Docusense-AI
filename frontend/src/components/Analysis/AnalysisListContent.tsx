...
export const AnalysisListContent: React.FC = () => {
  ...
  // Gestion du tri
  const handleSort = (field: 'created_at' | 'status' | 'provider' | 'file_name' | 'analysis_type') => {
    setSortParams(prev => ({
      ...prev,
      sort_by: field,
  ...
  // Gestion des filtres
  const handleFilterChange = (field: 'status' | 'prompt', value: string) => {
    setFilters(prev => ({
      ...prev,
      [field]: value
    }));
            ...
            {analyses.length === 0 && (
              <div className="mt-2 p-2 rounded-lg" style={{ 
                ...
                <p className="text-xs" style={{ color: colors.warning }}>
                  💡 Aucune analyse trouvée. Cliquez sur "Analyser avec l'IA" dans le menu contextuel d'un fichier pour commencer.
                </p>
                <div className="mt-2 flex items-center space-x-2">
                  <button
                    onClick={() => {
                      // Émettre un événement pour basculer vers la configuration
                      window.dispatchEvent(new CustomEvent('setActivePanel', { 
                        detail: { panel: 'config' } 
                      }));
                    }}
