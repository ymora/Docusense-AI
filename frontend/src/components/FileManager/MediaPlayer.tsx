...
const MediaPlayer: React.FC<MediaPlayerProps> = ({ file, onClose, onError }) => {
  ...
  const getStreamingUrl = () => {
    ...
    if (useFallback || (!isVideo && !isAudio)) {
      return `http://localhost:8000/api/files/stream-by-path/${encodeURIComponent(file.path)}?native=true${needsHls ? '&hls=true' : ''}&t=${Date.now()}`;
    }
              ...
              <ReactPlayer
                ...
                onError={(error) => {
                  ...
                  if ((isVideo || isAudio) && !useFallback) {
                    console.log('🔄 Tentative de fallback vers l\'ancien système de streaming...');
                    setUseFallback(true);
                    setHasError(false);
                    setIsLoading(true);
                    return;
